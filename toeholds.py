...
def alignment_toe_sense_antisense_matrix(library):
    ...
    for i in range(len(similar)):
        for j in range(i,len(similar)):
            vals = []
            vals.append(edit_distance_with_weighted_score(library[i], nu.reverse_complement(library[j]), exponential_weight))
            vals.append(edit_distance_with_weighted_score(library[j], nu.reverse_complement(library[i]), exponential_weight))
            val = np.min(vals)
            similar[i][j] = val
            similar[j][i] = val
